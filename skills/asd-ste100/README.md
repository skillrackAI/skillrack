...
From the repository root, copy the skill into your Claude Code skills directory:

```bash
cp -r skills/asd-ste100 ~/.claude/skills/
```

It has no dependencies on the rest of the repository, so this one directory is the whole skill.
...
claude-md/ste-strict.md           full STE for CLAUDE.md
claude-md/ste-custom-example.md   example profile with overrides
```
